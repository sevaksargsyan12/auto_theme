...
?>
    <h1 class="mt-4 h1">Աշխատանքներ ըստ բաժինների</h2>
    <div class="postBy-category-links">
        <a class="link-category custom-link-button <?php _e(($isFirstActive)?'active':'')?>" href="<?php _e($page_url)?>">
			Բոլորը
        </a>
        <?php foreach ($all_terms as $term):	$taxonomy_show= get_field('show_in_blog_and_single', $taxonomy.'_'.$term->term_id);
			if($taxonomy_show){ ?>
            <a class="link-category custom-link-button <?php _e(($termId == $term->term_id)?'active':'')?>" href="<?php _e('?service-category='.$term->slug)?>">
				<?php _e($term->name)?>
            </a>
        <?php } endforeach;?>
    </div>
...
if ($posts_by_category) {
	...
	foreach ($posts_by_category as $post) {
	 ...
	 echo ($i > 3 ? '<div class="col-lg-4 col-md-4 col-sm-12 cat-post d-none item-'.$i.'">':'<div class="col-lg-4 col-md-4  col-sm-12 cat-post active item-'.$i.'">');
	 	get_template_part('template-parts/post-item-large', 'post-item-large');
		echo '</div>';
        $i++;
	}
	echo '</div>'; if($post_count > 3):?>
    <button class="custom-button view-more cat w-auto m-auto" data-count="<?php _e($post_count)?>">Տեսնել ավելին<img src="<?php echo get_template_directory_uri();?>/images/right.svg" alt="button icon"/></button>
	<?php endif;?>
